...
def get_3dAbsAccel_arhsComplicated(accel_3d_list, omega_3d_list, t_interval_list):
    sample_rate = int(len(t_interval_list)/np.sum(t_interval_list))
...
def get_3dAbsAccel_rotMatrix(accel_3d_list, omega_3d_list, t_interval_list):
    rot_matrix = np.array([[1,0,0], [0,1,0],[0,0,1]])
    accel_3d_abs_list = []
    for index in range(t_interval_list.shape[0]):
        accel_3d_abs = np.matmul(rot_matrix, accel_3d_list[index])
        accel_3d_abs_list.append(accel_3d_abs)
        theta_x = omega_3d_list[index][0]*t_interval_list[index]
        theta_y = omega_3d_list[index][1]*t_interval_list[index]
        theta_z = omega_3d_list[index][2]*t_interval_list[index]
    ...
    return np.array(accel_3d_abs_list)
...
def noise_reduce(t_interval_list, accel_3d_list):
    timerange = np.sum(t_interval_list)
    nSampleSec = int(len(t_interval_list)/timerange)
    reduced_noise_raw = np.empty(accel_3d_list.shape)
    for axis in [0, 1, 2]:
        reduced_noise_raw[:, axis] = nr.reduce_noise(y=accel_3d_list[:, axis], sr=nSampleSec, n_std_thresh_stationary=1.5, stationary=True, time_mask_smooth_ms=timerange*1000)
    return reduced_noise_raw
...
def plot_3dAbsRotation_noEuler(omega_3d_list, accel_3d_list, accel_3d_abs_list, t_list):
    
    omega_degree_3d_list = np.degrees(omega_3d_list)
                              
    _, axes = plt.subplots(nrows=3, sharex=True, figsize=(15,15))
    ...
    axes[2].plot(t_list, accel_3d_abs_list[:, 0], "tab:red", label="X")
    axes[2].plot(t_list, accel_3d_abs_list[:, 1], "tab:green", label="Y")
    axes[2].plot(t_list, accel_3d_abs_list[:, 2], "tab:blue", label="Z")
    axes[2].set_title("Absolute Accel")
    axes[2].set_xlabel("Seconds")
